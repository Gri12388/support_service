...
//------------------------------------------------------------//
function InputText({ id, img, type, label, value, state, callbacks, placeholder } : { id : string, img? : string, type? : string, label : string, value : string, state : IinputElement, callbacks : IinputCallbacks, placeholder : string}) {
  
  //------------------------------------------------------------//
  // Массив, предназначенный для валидации поступившего типа
  // input элемента (аргумента type)                              
  //------------------------------------------------------------//
  const types : string[] = [ 'password', 'email' ];
